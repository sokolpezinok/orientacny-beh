...
const UserProfile: RenderComponent<typeof UserApi.user_profile> = memo(({ content, onUpdate }) => {
  const { t } = useTranslation();
  const { user_id } = useParams<{ user_id: string }>();
  const { actionFeedbackModal } = useModal();
  const formRef = useStatefulForm();

  const handleSubmit = actionFeedbackModal(async (data: Parameters<typeof UserApi.user_profile_update>[1]) => {
